...
class MyServer(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == "/register":
            newUser = {"username": formData["username"], "password": formData["password"]}
            register_User(newUser)
            self.wfile.write(bytes("User registered successfully.", "UTF-8"))
        else:
            if authentication(formData):
                self.wfile.write(bytes("Welcome, " + formData["username"], "UTF-8"))
            else:
